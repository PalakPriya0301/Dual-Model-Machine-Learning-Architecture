...
st.write("Search for a Customer ID to instantly generate a targeted campaign and dispatch it.")
    ...
    if not match.empty:
        ...
        if 'dynamic_rfm' in st.session_state:
            ...
            if not dyn_match.empty:
                st.session_state['active_customer'] = dyn_match.iloc[0]
                st.session_state['active_customer']['Persona'] = st.session_state['active_customer']['Assigned_Persona']
...
if 'active_customer' in st.session_state:
    ...
    if st.button("Draft & Send Targeted Email"):
        if not target_inbox:
            ...
            with st.spinner("Transmitting email via SMTP..."):
                ...
                if "Sleeper" in persona:
                    ...
                    body = f"Hi,\n\nIt's been {recency} days since your last purchase. We'd love to see you back!"
                elif "Top-Tier" in persona:
                    subject = "VIP Early Access: New Collection inside 🌟"
                    body = f"As one of our most loyal customers, you've earned exclusive early access."
                else:
                    subject = "Thinking of leveling up? Check out our Gold Tier 🥇"
                    body = f"Thanks for your {int(customer['Frequency'])} purchases! You're almost at VIP status."

                try:
                    msg = MIMEMultipart()
                    msg['From'], msg['To'], msg['Subject'] = SENDER_EMAIL, target_inbox, subject
